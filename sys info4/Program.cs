...
namespace SystemInfo
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            Console.Title = "系统信息检测工具（数智与信息化发展部V2026.0.3）";

            while (true)
            {
                Console.Clear();
                Console.WriteLine("========================================");
                Console.WriteLine("   系统信息检测工具（数智与信息化发展部V2026.0.3）");
                Console.WriteLine("========================================");
                Console.WriteLine("【核心亮点】");
                Console.WriteLine("  ✓ 支持基础/完整硬件配置检测");
                Console.WriteLine("  ✓ 一键保存报告到桌面（文件名含计算机名）");
                Console.WriteLine("  ✓ 实时逐行显示硬件信息，模拟命令行输出效果");
                Console.WriteLine("  ✓ 涵盖主板、BIOS、网卡、声卡、电池、显示器等详细信息");
                Console.WriteLine("  ✓ 品牌中文识别 + 出厂日期本地检测 + 信创产品判断");
                Console.WriteLine("  ✓ 自动识别台式机/笔记本 + 内存 DDR 代数及频率");
                Console.WriteLine("========================================");
        ...
        #region 信息收集模块
        static void CollectBasicInfo(StringBuilder sb)
        {
            AppendOperatingSystemInfo(sb);
            AppendComputerSystemInfo(sb);
            AppendProcessorInfo(sb);
            AppendMemoryInfo(sb);       // 已增强：DDR代数+频率
            AppendDiskInfo(sb);
            AppendVideoControllerInfo(sb);
        }

        static void CollectFullInfo(StringBuilder sb)
        {
            AppendOperatingSystemInfo(sb);
            AppendComputerSystemInfo(sb);
            AppendProcessorInfo(sb);
            ...
            AppendSoundDeviceInfo(sb);
            AppendBatteryInfo(sb);
            AppendMonitorInfo(sb);
        }
        #endregion
        ...
        #region 计算机品牌与型号（含中文名称、设备类型、出厂日期、信创判断）
        static void AppendComputerSystemInfo(StringBuilder sb)
        {
            sb.AppendLine("========== 计算机品牌与型号 ==========");
            string manufacturer = "";
            string model = "";
            int pcSystemType = 0; // 1=台式机，2=笔记本，3=服务器等
            using (var searcher = new ManagementObjectSearcher("SELECT * FROM Win32_ComputerSystem"))
            {
                foreach (ManagementObject cs in searcher.Get())
                {
                    manufacturer = cs["Manufacturer"]?.ToString().Trim() ?? "未知";
                    model = cs["Model"]?.ToString().Trim() ?? "未知";
                    if (cs["PCSystemType"] != null)
                        pcSystemType = Convert.ToInt32(cs["PCSystemType"]);
                    string manufacturerCn = GetChineseBrandName(manufacturer);
                    sb.AppendLine($"制造商: {manufacturer}（{manufacturerCn}）");
                    // 型号后追加设备类型
                    string deviceType = GetDeviceType(pcSystemType);
                    sb.AppendLine($"型号: {model} [{deviceType}]");
                }
            }
            string serialNumber = "";
            string uuid = "";
        ...
        static string GetDeviceType(int pcSystemType)
        {
            switch (pcSystemType)
            {
                case 1: return "台式机";
        ...
        static void AppendMemoryInfo(StringBuilder sb)
        {
            ...
            using (var searcher = new ManagementObjectSearcher("SELECT * FROM Win32_PhysicalMemory"))
            {
                foreach (ManagementObject mem in searcher.Get())
                {
                    ...
                    uint speed = 0;
                    if (mem["Speed"] != null)
                        speed = Convert.ToUInt32(mem["Speed"]);
                    // 获取内存类型（DDR代数）
                    int memoryType = 0;
                    if (mem["MemoryType"] != null)
                        memoryType = Convert.ToInt32(mem["MemoryType"]);
                    string ddrType = GetDDRType(memoryType);
                    sb.AppendLine($"插槽{slotIndex}: {capacity:F2} GB  {ddrType}  {speed} MHz");
                }
        ...
        /// <summary>根据 MemoryType 值返回 DDR 代数文本</summary>
        static string GetDDRType(int memoryType)
        {
            // 常见的 MemoryType 值参考：20=DDR，21=DDR2，24=DDR3，26=DDR4，34=DDR5
            switch (memoryType)
            {
                case 20: return "DDR";
                case 21: return "DDR2";
                ...
                case 26: return "DDR4";
                case 34: return "DDR5";
                default: return memoryType > 0 ? $"类型代码{memoryType}" : "未知类型";
            }
        }
        #endregion
